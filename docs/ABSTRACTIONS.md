...
1. Reads cache from `~/.bigfoot/cache/<vault-hash>.json` (external to vault)
...
7. On first run, migrates any legacy `.bigfoot-cache.json` from inside the vault
...
- Persists vault list to `~/.config/com.bigfoot.app/vaults.json` (reads legacy `com.bigfoot.app` on upgrade)
...
Installation-local layout state that should not sync through a vault stays in localStorage. `useLayoutPanels` stores the clamped sidebar, note-list, and inspector widths under `bigfoot:layout-panels` so pane sizing survives app relaunches on the same machine.
...
App-level settings persisted at `~/.config/com.bigfoot.app/settings.json` (reads legacy `com.bigfoot.app` on upgrade):
