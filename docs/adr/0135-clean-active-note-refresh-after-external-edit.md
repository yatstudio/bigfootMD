...
Bigfoot Note's filesystem-first model requires clean in-memory editor state to converge to the file on disk during the current session. Unsaved local editor buffers still need protection, but editor focus alone is not enough reason to keep showing stale content when the changed-path batch identifies the active file.
...
- External edits to the currently open clean note become visible without restarting Bigfoot Note.
