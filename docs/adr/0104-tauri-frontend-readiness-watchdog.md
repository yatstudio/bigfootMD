...
Bigfoot Note already keeps heavy filesystem and subprocess work off the Tauri window-creation path, but that alone does not protect against a different startup failure mode: the desktop WebView can render the static HTML shell while the React app never becomes interactive.
...
Bigfoot Note needs a startup contract that distinguishes “HTML painted” from “frontend actually became interactive”, and a bounded recovery path when that contract is not satisfied.

## Decision

**Bigfoot Note uses a Tauri-only frontend-readiness watchdog that reloads the WebView at most once if React never reports startup readiness.**
...
- `sessionStorage` tracks whether the startup reload was already attempted so Bigfoot Note does not loop forever
...
- Bigfoot Note now distinguishes successful frontend startup from merely rendering the HTML shell.
...
- If a startup failure persists after one retry, Bigfoot Note still surfaces the broken state instead of hiding a deeper bug behind repeated reloads.
