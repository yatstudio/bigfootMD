...
- Git is now a first-class prerequisite for Bigfoot Note vaults, not just implied by the cache strategy.
...
- Re-evaluate if Bigfoot Note needs to support non-git vaults (e.g., iCloud-only, shared network drive); at that point ADR-0014 would also need revisiting.
