...
A note is a Markdown file with optional YAML frontmatter. Bigfoot Note reads the first H1 as the primary title and keeps the file on disk as the durable representation.
...
Draft the public Bigfoot Note docs and keep them close to code changes.
```

## Titles

The first H1 is the note title. Bigfoot Note uses that title wherever the note is displayed: note lists, search results, wikilink suggestions, relationship pickers, tabs, and window titles.
...
Use `[[wikilinks]]` to connect notes from the body. Bigfoot Note shows autocomplete suggestions while you type, and links can resolve by filename or title.
