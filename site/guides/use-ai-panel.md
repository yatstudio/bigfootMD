...
Bigfoot Note gives you two ways to ask for AI help: open the AI panel for an ongoing conversation, or prompt directly from the editor with `Cmd+K` followed by a space.
...
If a coding agent is missing, install it and reopen Bigfoot Note or switch to another target.
...
AI edits are file edits. Review them with Bigfoot Note's diff and Git history before committing.
